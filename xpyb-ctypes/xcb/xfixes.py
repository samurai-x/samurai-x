...
class SelectionNotifyEvent(xcb.Event):
    def __init__(self, parent):
        ...
        count = 0
        (self.subtype, self.window, self.owner, self.selection, self.timestamp, self.selection_timestamp,) = unpack_ex('xB2xIIIII8x', self, count)
...
class CursorNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.subtype, self.window, self.cursor_serial, self.timestamp, self.name,) = unpack_ex('xB2xIIII12x', self, count)
...
class GetCursorImageReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.x, self.y, self.width, self.height, self.xhot, self.yhot, self.cursor_serial,) = unpack_ex('xx2x4xhhHHHHI8x', self, count)
        count += 32
...
class GetCursorNameReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.atom, self.nbytes,) = unpack_ex('xx2x4xIH18x', self, count)
        count += 32
...
class GetCursorImageAndNameReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.x, self.y, self.width, self.height, self.xhot, self.yhot, self.cursor_serial, self.cursor_atom, self.nbytes,) = unpack_ex('xx2x4xhhHHHHIIH2x', self, count)
        count += 32
        self.name = xcb.List(self, count, self.nbytes, 'B', 1)
