...
class INT64(xcb.Struct):
    def __init__(self, parent, offset, size):
        ...
        count = 0
        (self.hi, self.lo,) = unpack_ex('iI', self, count)
...
class TRIGGER(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.counter, self.wait_type,) = unpack_ex('II', self, count)
        count += 8
        self.wait_value = INT64(self, count, 8)
        count += 8
        count += xcb.type_pad(4, count)
        (self.test_type,) = unpack_ex('I', self, count)
...
class CounterError(xcb.Error):
    def __init__(self, parent):
        ...
        count = 0
        (self.bad_counter, self.minor_opcode, self.major_opcode,) = unpack_ex('xx2xIHB', self, count)
...
class AlarmError(xcb.Error):
    def __init__(self, parent):
        xcb.Error.__init__(self, parent)
        count = 0
        (self.bad_alarm, self.minor_opcode, self.major_opcode,) = unpack_ex('xx2xIHB', self, count)
...
class InitializeReply(xcb.Reply):
    def __init__(self, parent):
        ...
        count = 0
        (self.major_version, self.minor_version,) = unpack_ex('xx2x4xBB', self, count)
...
class ListSystemCountersReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.counters_len,) = unpack_ex('xx2x4xI20x', self, count)
        count += 32
...
class GetPriorityReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.priority,) = unpack_ex('xx2x4xi', self, count)
