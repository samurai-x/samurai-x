...
class NotifyEvent(xcb.Event):
    def __init__(self, parent):
        ...
        count = 0
        (self.shape_kind, self.affected_window, self.extents_x, self.extents_y, self.extents_width, self.extents_height, self.server_time, self.shaped,) = unpack_ex('xB2xIhhHHIB', self, count)
...
class InputSelectedReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.enabled,) = unpack_ex('xB2x4x', self, count)
...
class GetRectanglesReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.ordering, self.rectangles_len,) = unpack_ex('xB2x4xI', self, count)
        count += 12
...
class shapeExtension(xcb.Extension):
    ...
    def SelectInputChecked(self, destination_window, enable):
        buf = cStringIO.StringIO()
        buf.write(pack('xx2xIB', destination_window, enable))
    ...
    def GetRectangles(self, window, source_kind):
        buf = cStringIO.StringIO()
        buf.write(pack('xx2xIB', window, source_kind))
