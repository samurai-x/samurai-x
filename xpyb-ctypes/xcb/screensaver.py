...
class QueryVersionReply(xcb.Reply):
    def __init__(self, parent):
        ...
        count = 0
        (self.server_major_version, self.server_minor_version,) = unpack_ex('xx2x4xHH20x', self, count)
...
class QueryInfoReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.state, self.saver_window, self.ms_until_server, self.ms_since_user_input, self.event_mask, self.kind,) = unpack_ex('xB2x4xIIIIB7x', self, count)

class NotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.code, self.state, self.sequence_number, self.time, self.root, self.window, self.kind, self.forced,) = unpack_ex('xB2xBHIIIBB14x', self, count)
