...
class CHAR2B(xcb.Struct):
    def __init__(self, parent, offset, size):
        ...
        count = 0
        (self.byte1, self.byte2,) = unpack_ex('BB', self, count)

class POINT(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.x, self.y,) = unpack_ex('hh', self, count)

class RECTANGLE(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.x, self.y, self.width, self.height,) = unpack_ex('hhHH', self, count)

class ARC(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.x, self.y, self.width, self.height, self.angle1, self.angle2,) = unpack_ex('hhHHhh', self, count)

class FORMAT(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.depth, self.bits_per_pixel, self.scanline_pad,) = unpack_ex('BBB5x', self, count)
...
class VISUALTYPE(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.visual_id, self._class, self.bits_per_rgb_value, self.colormap_entries, self.red_mask, self.green_mask, self.blue_mask,) = unpack_ex('IBBHIII4x', self, count)

class DEPTH(xcb.Struct):
    def __init__(self, parent, offset):
        xcb.Struct.__init__(self, parent, offset)
        count = 0
        (self.depth, self.visuals_len,) = unpack_ex('BxH4x', self, count)
        count += 8
...
class SetupRequest(xcb.Struct):
    def __init__(self, parent, offset):
        xcb.Struct.__init__(self, parent, offset)
        count = 0
        (self.byte_order, self.protocol_major_version, self.protocol_minor_version, self.authorization_protocol_name_len, self.authorization_protocol_data_len,) = unpack_ex('BxHHHH', self, count)
        count += 10
...
class SetupFailed(xcb.Struct):
    def __init__(self, parent, offset):
        xcb.Struct.__init__(self, parent, offset)
        count = 0
        (self.status, self.reason_len, self.protocol_major_version, self.protocol_minor_version, self.length,) = unpack_ex('BBHHH', self, count)
        count += 8
...
class SetupAuthenticate(xcb.Struct):
    def __init__(self, parent, offset):
        xcb.Struct.__init__(self, parent, offset)
        count = 0
        (self.status, self.length,) = unpack_ex('B5xH', self, count)
        count += 8
...
class KeyPressEvent(xcb.Event):
    def __init__(self, parent):
        ...
        count = 0
        (self.detail, self.time, self.root, self.event, self.child, self.root_x, self.root_y, self.event_x, self.event_y, self.state, self.same_screen,) = unpack_ex('xB2xIIIIhhhhHB', self, count)
...
class EnterNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.detail, self.time, self.root, self.event, self.child, self.root_x, self.root_y, self.event_x, self.event_y, self.state, self.mode, self.same_screen_focus,) = unpack_ex('xB2xIIIIhhhhHBB', self, count)
...
class FocusInEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.detail, self.event, self.mode,) = unpack_ex('xB2xIB', self, count)
...
class ExposeEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.window, self.x, self.y, self.width, self.height, self.count,) = unpack_ex('xx2xIHHHHH', self, count)

class GraphicsExposureEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.drawable, self.x, self.y, self.width, self.height, self.minor_opcode, self.count, self.major_opcode,) = unpack_ex('xx2xIHHHHHHB', self, count)

class NoExposureEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.drawable, self.minor_opcode, self.major_opcode,) = unpack_ex('xx2xIHB', self, count)
...
class VisibilityNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.window, self.state,) = unpack_ex('xx2xIB', self, count)

class CreateNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.parent, self.window, self.x, self.y, self.width, self.height, self.border_width, self.override_redirect,) = unpack_ex('xx2xIIhhHHHB', self, count)

class DestroyNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.event, self.window,) = unpack_ex('xx2xII', self, count)

class UnmapNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.event, self.window, self.from_configure,) = unpack_ex('xx2xIIB', self, count)

class MapNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.event, self.window, self.override_redirect,) = unpack_ex('xx2xIIB', self, count)

class MapRequestEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.parent, self.window,) = unpack_ex('xx2xII', self, count)

class ReparentNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.event, self.window, self.parent, self.x, self.y, self.override_redirect,) = unpack_ex('xx2xIIIhhB', self, count)

class ConfigureNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.event, self.window, self.above_sibling, self.x, self.y, self.width, self.height, self.border_width, self.override_redirect,) = unpack_ex('xx2xIIIhhHHHB', self, count)

class ConfigureRequestEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.stack_mode, self.parent, self.window, self.sibling, self.x, self.y, self.width, self.height, self.border_width, self.value_mask,) = unpack_ex('xB2xIIIhhHHHH', self, count)

class GravityNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.event, self.window, self.x, self.y,) = unpack_ex('xx2xIIhh', self, count)

class ResizeRequestEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.window, self.width, self.height,) = unpack_ex('xx2xIHH', self, count)
...
class CirculateNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.event, self.window, self.place,) = unpack_ex('xx2xII4xB', self, count)
...
class PropertyNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.window, self.atom, self.time, self.state,) = unpack_ex('xx2xIIIB', self, count)

class SelectionClearEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.time, self.owner, self.selection,) = unpack_ex('xx2xIII', self, count)

class SelectionRequestEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.time, self.owner, self.requestor, self.selection, self.target, self.property,) = unpack_ex('xx2xIIIIII', self, count)

class SelectionNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.time, self.requestor, self.selection, self.target, self.property,) = unpack_ex('xx2xIIIII', self, count)
...
class ColormapNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.window, self.colormap, self.new, self.state,) = unpack_ex('xx2xIIBB', self, count)
...
class ClientMessageEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.format, self.window, self.type,) = unpack_ex('xB2xII', self, count)
        count += 12
...
class MappingNotifyEvent(xcb.Event):
    def __init__(self, parent):
        xcb.Event.__init__(self, parent)
        count = 0
        (self.request, self.first_keycode, self.count,) = unpack_ex('xx2xBBB', self, count)

class RequestError(xcb.Error):
    def __init__(self, parent):
        xcb.Error.__init__(self, parent)
        count = 0
        (self.bad_value, self.minor_opcode, self.major_opcode,) = unpack_ex('xx2xIHB', self, count)
...
class GetGeometryReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.depth, self.root, self.x, self.y, self.width, self.height, self.border_width,) = unpack_ex('xB2x4xIhhHHH', self, count)
...
class QueryTreeReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.root, self.parent, self.children_len,) = unpack_ex('xx2x4xIIH14x', self, count)
        count += 32
...
class InternAtomReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.atom,) = unpack_ex('xx2x4xI', self, count)
...
class GetAtomNameReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.name_len,) = unpack_ex('xx2x4xH22x', self, count)
        count += 32
...
class GetPropertyReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.format, self.type, self.bytes_after, self.value_len,) = unpack_ex('xB2x4xIII12x', self, count)
        count += 32
        self.value = xcb.List(self, count, self.value_len, 'B', 1)
...
class ListPropertiesReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.atoms_len,) = unpack_ex('xx2x4xH22x', self, count)
        count += 32
...
class GetSelectionOwnerReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.owner,) = unpack_ex('xx2x4xI', self, count)
...
class GrabPointerReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.status,) = unpack_ex('xB2x4x', self, count)
...
class QueryPointerReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.same_screen, self.root, self.child, self.root_x, self.root_y, self.win_x, self.win_y, self.mask,) = unpack_ex('xB2x4xIIhhhhH', self, count)

class TIMECOORD(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.time, self.x, self.y,) = unpack_ex('Ihh', self, count)
...
class GetMotionEventsReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.events_len,) = unpack_ex('xx2x4xI20x', self, count)
        count += 32
...
class TranslateCoordinatesReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.same_screen, self.child, self.dst_x, self.dst_y,) = unpack_ex('xB2x4xIHH', self, count)
...
class GetInputFocusReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.revert_to, self.focus,) = unpack_ex('xB2x4xI', self, count)
...
class FONTPROP(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.name, self.value,) = unpack_ex('II', self, count)

class CHARINFO(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.left_side_bearing, self.right_side_bearing, self.character_width, self.ascent, self.descent, self.attributes,) = unpack_ex('hhhhhH', self, count)
...
class QueryFontReply(xcb.Reply):
    def __init__(self, parent):
        ...
        count += 12
        (self.min_char_or_byte2, self.max_char_or_byte2, self.default_char, self.properties_len, self.draw_direction, self.min_byte1, self.max_byte1, self.all_chars_exist, self.font_ascent, self.font_descent, self.char_infos_len,) = unpack_ex('4xHHHHBBBBhhI', self, count)
        count += 24
        count += xcb.type_pad(8, count)
...
class QueryTextExtentsReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.draw_direction, self.font_ascent, self.font_descent, self.overall_ascent, self.overall_descent, self.overall_width, self.overall_left, self.overall_right,) = unpack_ex('xB2x4xhhhhiii', self, count)

class STR(xcb.Struct):
    def __init__(self, parent, offset):
        xcb.Struct.__init__(self, parent, offset)
        count = 0
        (self.name_len,) = unpack_ex('B', self, count)
        count += 1
        self.name = xcb.List(self, count, self.name_len, 'b', 1)
...
class ListFontsReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.names_len,) = unpack_ex('xx2x4xH22x', self, count)
        count += 32
...
class ListFontsWithInfoReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.name_len,) = unpack_ex('xB2x4x', self, count)
        count += 8
        self.min_bounds = CHARINFO(self, count, 12)
        count += 12
        count += 4
        ...
        self.max_bounds = CHARINFO(self, count, 12)
        count += 12
        (self.min_char_or_byte2, self.max_char_or_byte2, self.default_char, self.properties_len, self.draw_direction, self.min_byte1, self.max_byte1, self.all_chars_exist, self.font_ascent, self.font_descent, self.replies_hint,) = unpack_ex('4xHHHHBBBBhhI', self, count)
        count += 24
        count += xcb.type_pad(8, count)
        self.properties = xcb.List(self, count, self.properties_len, FONTPROP, 8)
        count += len(self.properties.buf())
...
class GetFontPathReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.path_len,) = unpack_ex('xx2x4xH22x', self, count)
        count += 32
...
class SEGMENT(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.x1, self.y1, self.x2, self.y2,) = unpack_ex('hhhh', self, count)
...
class GetImageReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.depth, self.visual,) = unpack_ex('xB2x4xI20x', self, count)
        count += 32
...
class ListInstalledColormapsReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.cmaps_len,) = unpack_ex('xx2x4xH22x', self, count)
        count += 32
...
class AllocColorReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.red, self.green, self.blue, self.pixel,) = unpack_ex('xx2x4xHHH2xI', self, count)
...
class AllocNamedColorReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.pixel, self.exact_red, self.exact_green, self.exact_blue, self.visual_red, self.visual_green, self.visual_blue,) = unpack_ex('xx2x4xIHHHHHH', self, count)
...
class AllocColorCellsReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.pixels_len, self.masks_len,) = unpack_ex('xx2x4xHH20x', self, count)
        count += 32
...
class AllocColorPlanesReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.pixels_len, self.red_mask, self.green_mask, self.blue_mask,) = unpack_ex('xx2x4xH2xIII8x', self, count)
        count += 32
        self.pixels = xcb.List(self, count, self.pixels_len, 'I', 4)
...
class COLORITEM(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.pixel, self.red, self.green, self.blue, self.flags,) = unpack_ex('IHHHBx', self, count)

class RGB(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.red, self.green, self.blue,) = unpack_ex('HHH2x', self, count)
...
class QueryColorsReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.colors_len,) = unpack_ex('xx2x4xH22x', self, count)
        count += 32
...
class LookupColorReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.exact_red, self.exact_green, self.exact_blue, self.visual_red, self.visual_green, self.visual_blue,) = unpack_ex('xx2x4xHHHHHH', self, count)
...
class QueryBestSizeReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.width, self.height,) = unpack_ex('xx2x4xHH', self, count)
...
class QueryExtensionReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.present, self.major_opcode, self.first_event, self.first_error,) = unpack_ex('xx2x4xBBBB', self, count)
...
class ListExtensionsReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.names_len,) = unpack_ex('xB2x4x24x', self, count)
        count += 32
        self.names = xcb.List(self, count, self.names_len, STR, -1)
...
class GetKeyboardMappingReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.keysyms_per_keycode,) = unpack_ex('xB2x4x24x', self, count)
        count += 32
...
class GetKeyboardControlReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.global_auto_repeat, self.led_mask, self.key_click_percent, self.bell_percent, self.bell_pitch, self.bell_duration,) = unpack_ex('xB2x4xIBBHH2x', self, count)
        count += 20
...
class GetPointerControlReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.acceleration_numerator, self.acceleration_denominator, self.threshold,) = unpack_ex('xx2x4xHHH', self, count)
...
class GetScreenSaverReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.timeout, self.interval, self.prefer_blanking, self.allow_exposures,) = unpack_ex('xx2x4xHHBB', self, count)
...
class HOST(xcb.Struct):
    def __init__(self, parent, offset):
        xcb.Struct.__init__(self, parent, offset)
        count = 0
        (self.family, self.address_len,) = unpack_ex('BxH', self, count)
        count += 4
...
class ListHostsReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.mode, self.hosts_len,) = unpack_ex('xB2x4xH22x', self, count)
        count += 32
...
class GetPointerMappingReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.map_len,) = unpack_ex('xB2x4x24x', self, count)
        count += 32
...
class GetModifierMappingReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.keycodes_per_modifier,) = unpack_ex('xB2x4x24x', self, count)
        count += 32
...
class xprotoExtension(xcb.Extension):
    ...
    def ChangeActivePointerGrabChecked(self, cursor, time, event_mask):
        buf = cStringIO.StringIO()
        buf.write(pack('xx2xIIH', cursor, time, event_mask))
    ...
    def GrabKeyboard(self, owner_events, grab_window, time, pointer_mode, keyboard_mode):
        buf = cStringIO.StringIO()
        buf.write(pack('xB2xIIBB', owner_events, grab_window, time, pointer_mode, keyboard_mode))
    ...
    def GrabKeyChecked(self, owner_events, grab_window, modifiers, key, pointer_mode, keyboard_mode):
        buf = cStringIO.StringIO()
        buf.write(pack('xB2xIHBBB', owner_events, grab_window, modifiers, key, pointer_mode, keyboard_mode))
    ...
    def UngrabKeyChecked(self, key, grab_window, modifiers):
        buf = cStringIO.StringIO()
        buf.write(pack('xB2xIH', key, grab_window, modifiers))
    ...
    def OpenFontChecked(self, fid, name_len, name):
        buf = cStringIO.StringIO()
        buf.write(pack('xx2xIH', fid, name_len))
        buf.write(str(buffer(array('b', name))))
    ...
    def FillPolyChecked(self, drawable, gc, shape, coordinate_mode, points_len, points):
        buf = cStringIO.StringIO()
        buf.write(pack('xx2xIIBB', drawable, gc, shape, coordinate_mode))
        for elt in xcb.Iterator(points, 2, 'points', True):
            buf.write(pack('hh', *elt))
    ...
    def PutImageChecked(self, format, drawable, gc, width, height, dst_x, dst_y, left_pad, depth, data_len, data):
        buf = cStringIO.StringIO()
        buf.write(pack('xB2xIIHHhhBB', format, drawable, gc, width, height, dst_x, dst_y, left_pad, depth))
        buf.write(str(buffer(array('B', data))))
    ...
    def AllocColor(self, cmap, red, green, blue):
        buf = cStringIO.StringIO()
        buf.write(pack('xx2xIHHH', cmap, red, green, blue))
    ...
    def AllocNamedColor(self, cmap, name_len, name):
        buf = cStringIO.StringIO()
        buf.write(pack('xx2xIH', cmap, name_len))
        buf.write(str(buffer(array('b', name))))
    ...
    def StoreNamedColorChecked(self, flags, cmap, pixel, name_len, name):
        buf = cStringIO.StringIO()
        buf.write(pack('xB2xIIH', flags, cmap, pixel, name_len))
        buf.write(str(buffer(array('b', name))))
    ...
    def RotatePropertiesChecked(self, window, atoms_len, delta, atoms):
        buf = cStringIO.StringIO()
        buf.write(pack('xI2xHh', window, atoms_len, delta))
