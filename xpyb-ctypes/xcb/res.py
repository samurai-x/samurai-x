...
class Client(xcb.Struct):
    def __init__(self, parent, offset, size):
        ...
        count = 0
        (self.resource_base, self.resource_mask,) = unpack_ex('II', self, count)

class Type(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.resource_type, self.count,) = unpack_ex('II', self, count)
...
class QueryVersionReply(xcb.Reply):
    def __init__(self, parent):
        ...
        count = 0
        (self.server_major, self.server_minor,) = unpack_ex('xx2x4xHH', self, count)
...
class QueryClientsReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.num_clients,) = unpack_ex('xx2x4xI20x', self, count)
        count += 32
...
class QueryClientResourcesReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.num_types,) = unpack_ex('xx2x4xI20x', self, count)
        count += 32
...
class QueryClientPixmapBytesReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.bytes, self.bytes_overflow,) = unpack_ex('xx2x4xII', self, count)
