...
class Range8(xcb.Struct):
    def __init__(self, parent, offset, size):
        ...
        count = 0
        (self.first, self.last,) = unpack_ex('BB', self, count)

class Range16(xcb.Struct):
    def __init__(self, parent, offset, size):
        xcb.Struct.__init__(self, parent, offset, size)
        count = 0
        (self.first, self.last,) = unpack_ex('HH', self, count)
...
class Range(xcb.Struct):
    def __init__(self, parent, offset, size):
        ...
        count += 2
        count += xcb.type_pad(4, count)
        (self.client_started, self.client_died,) = unpack_ex('BB', self, count)
...
class ClientInfo(xcb.Struct):
    def __init__(self, parent, offset):
        ...
        count = 0
        (self.client_resource, self.num_ranges,) = unpack_ex('II', self, count)
        count += 8
...
class ContextError(xcb.Error):
    def __init__(self, parent):
        ...
        count = 0
        (self.invalid_record,) = unpack_ex('xx2xI', self, count)
...
class GetContextReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.enabled, self.element_header, self.num_intercepted_clients,) = unpack_ex('xB2x4xB3xI16x', self, count)
        count += 32
...
class EnableContextReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.category, self.element_header, self.client_swapped, self.xid_base, self.server_time, self.rec_sequence_num,) = unpack_ex('xB2x4xBB2xIII8x', self, count)
        count += 32
