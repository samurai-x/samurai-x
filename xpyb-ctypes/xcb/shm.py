...
class CompletionEvent(xcb.Event):
    def __init__(self, parent):
        ...
        count = 0
        (self.drawable, self.shmseg, self.minor_event, self.major_event, self.offset,) = unpack_ex('xx2xIIHBxI', self, count)
...
class QueryVersionReply(xcb.Reply):
    def __init__(self, parent):
        ...
        count = 0
        (self.shared_pixmaps, self.major_version, self.minor_version, self.uid, self.gid, self.pixmap_format,) = unpack_ex('xB2x4xHHHHB', self, count)
...
class GetImageReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.depth, self.visual, self.size,) = unpack_ex('xB2x4xII', self, count)
...
class shmExtension(xcb.Extension):
    ...
    def AttachChecked(self, shmseg, shmid, read_only):
        buf = cStringIO.StringIO()
        buf.write(pack('xx2xIIB', shmseg, shmid, read_only))
