...
class SurfaceInfo(xcb.Struct):
    def __init__(self, parent, offset, size):
        ...
        count = 0
        (self.id, self.chroma_format, self.pad0, self.max_width, self.max_height, self.subpicture_max_width, self.subpicture_max_height, self.mc_type, self.flags,) = unpack_ex('IHHHHHHII', self, count)
...
class QueryVersionReply(xcb.Reply):
    def __init__(self, parent):
        ...
        count = 0
        (self.major, self.minor,) = unpack_ex('xx2x4xII', self, count)
...
class ListSurfaceTypesReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.num,) = unpack_ex('xx2x4xI20x', self, count)
        count += 32
...
class CreateContextReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.width_actual, self.height_actual, self.flags_return,) = unpack_ex('xx2x4xHHI20x', self, count)
        count += 36
...
class CreateSubpictureReply(xcb.Reply):
    def __init__(self, parent):
        xcb.Reply.__init__(self, parent)
        count = 0
        (self.width_actual, self.height_actual, self.num_palette_entries, self.entry_bytes,) = unpack_ex('xx2x4xHHHH', self, count)
        count += 16
