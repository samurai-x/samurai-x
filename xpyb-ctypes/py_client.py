...
def py_open(self):
    ...
    _py('import ctypes')
    _py('from struct import pack, unpack_from, calcsize')
    _py('from array import array')
    _py('')
    _py('def unpack_ex(fmt, protobj, offset=0):\n'
...
def _py_complex(self, name):
    ...
    for field in self.fields:
        ...
        if len(list) > 0:
            _py('        (%s,) = unpack_ex(\'%s\', self, count)', list, format)
        if size > 0:
            _py('        count += %d', size)
...
def py_union(self, name):
    ...
    for field in self.fields:
        if field.type.is_simple:
            _py('        self.%s = unpack_ex(\'%s\', self)', _n(field.field_name), field.type.py_format_str)
